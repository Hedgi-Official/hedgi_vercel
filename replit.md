...
## Changelog
- February 7, 2026. **Codebase cleanup Phase 1 & 2** - Removed dead/legacy code: Next.js API stubs (pages/api/payment/), deprecated server files (routes-clean.ts, start-services.ts, services/trading.ts, services/marketClosedRetryService.ts), deprecated XTB client files (xtb-service.ts, xtb-types.ts). Moved unrouted pages to archive (landing-page.tsx, for-individuals.tsx, using-hedgi.tsx, landing-page-archived-sections.tsx). Removed 10 orphaned components no longer imported by active code (building-backdrop, skyline, typing-effect, CurrencyNewsFeed, under-construction, flask-payment-modal, MercadoPayIframeModal, enhanced-currency-simulator, standalone-window-payment, currency-chart). Cleaned up related CSS. Remaining cleanup candidates: server/routes.ts vs routes-fixed.ts consolidation, server/simple-auth.ts review, i18n translation pruning (Phase 3 & 4).
- February 3, 2026. **SEO improvements implemented** - Added comprehensive meta tags (description, keywords, Open Graph, Twitter Cards), created robots.txt and sitemap.xml for search engines, integrated react-helmet-async for dynamic page titles, added JSON-LD structured data (Organization and WebApplication schemas), and implemented configurable canonical URLs via VITE_BASE_URL environment variable. Added multi-language SEO support with translated meta descriptions for English and Portuguese, dynamic og:locale, and HTML lang attribute updates based on i18n language. Note: hreflang tags require URL-based language routing for full implementation (future enhancement).
- February 2, 2026. **API-managed pending orders and dashboard update** - Disabled the 10-minute retry service since Hedgi API now handles pending orders automatically. Updated corporate dashboard to display two types of pending orders: (1) API Pending - orders stored in Hedgi API waiting for market to open, auto-executed when markets open; (2) Scheduled Orders - orders from batch uploads in local pending_orders table. Added translations for new pending order sections in English and Portuguese.
- January 26, 2026. **Archived About Us page** - Moved about-us.tsx to client/src/pages/archive/ for future retrieval. Removed route from App.tsx and navigation links from header (both desktop and mobile).
- January 26, 2026. **Fixed spread cost currency display** - Corporate dashboard now correctly converts spread/swap/total costs to USD for USDBRL and USDMXN pairs by dividing quote currency values by bid rate. EURUSD costs remain unchanged (already in USD). Shows original quote currency values in parentheses for clarity.
- January 26, 2026. **AI Assistant for corporate hedge simulation** - Added collapsible AI chat assistant to the corporate dashboard's Simulate tab. Users can describe their hedge needs in natural language (e.g., "hedge $50,000 USD against BRL for 2 weeks"), and the AI extracts symbol, direction, volume, and duration to auto-fill the simulation form. Uses existing OpenAI gpt-4o-mini integration with structured JSON output parsing. Includes session management and form validation.
- January 16, 2026. **Enhanced batch execution with failure routing and scheduled adjustments** - Failed orders now get routed to pending queue with parsed error codes for retry. Timeline netting automatically saves future adjustment orders to pending queue. Drag-and-drop file upload fixed with visual feedback. Rate fetching improved with better error handling for synthetic pair decomposition.
...
- February 2, 2026. **Security fix: Replaced vulnerable xlsx package** - Replaced xlsx@0.18.5 (contains critical CVEs) with exceljs for Excel file parsing in batch upload functionality
