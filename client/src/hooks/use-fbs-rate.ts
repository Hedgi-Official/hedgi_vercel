...
export function useFBSRate(symbol: string = 'USDBRL') {
  return useQuery<FBSRateResponse, Error>({
    ...
    queryFn: async () => {
      ...
      try {
        const response = await fetch(`/api/fbs-broker-rate?symbol=${symbol}`);
        if (response.ok) {
          const data = await response.json();
          console.log("[useFBSRate] Rate data from new endpoint:", data);
          return data;
        }
      } catch (error) {
        console.error("[useFBSRate] Error fetching from new endpoint:", error);
      }
      ...
      const data = await response.json();
      console.log("[useFBSRate] Rate data from original endpoint:", data);
      return data;
    },
