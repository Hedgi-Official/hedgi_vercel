...
export function ExchangeRatesWidget() {
  ...
  return (
    <Card className="bg-background shadow-lg">
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            {(isLoadingActivTrades || isLoadingTickmill) && <Loader2 className="h-4 w-4 animate-spin" />}
          </div>
      ...
      <CardContent>
        {!activTradesRate && !tickmillRate && (activTradesError || tickmillError) ? (
          <div className="text-destructive">
            Error loading rates: {activTradesError?.message || tickmillError?.message}
          </div>
