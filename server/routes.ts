...
export function registerRoutes(app: Express): Server {
  ...
  app.post("/api/hedges", async (req, res) => {
    ...
    try {
      ...
      // Record the trade in the database with the serial userId field
      // We need to use a query to insert with a specific userId since it's a serial field
      const [hedge] = await db
        .insert(hedges)
        .values({
          // userId will be automatically generated by the database since it's a serial field
          baseCurrency,
          targetCurrency,
          ...
          status: "active",
          tradeOrderNumber: Number(tradeOrderNumber), // Store as number
          tradeStatus: "ACTIVE", // Assuming 'ACTIVE' upon successful trade opening.
        })
        .returning();
        
      // Update the userId to match the logged-in user
      if (hedge && hedge.id) {
        await db
          .update(hedges)
          .set({ userId: Number(req.user.id) })
