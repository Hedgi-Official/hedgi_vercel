...
async function fetchWithRetry(url: string, options: any, maxRetries = 3): Promise<any> {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.log(`[Tickmill] Attempt ${attempt} failed, retrying...`);
      await new Promise(resolve => setTimeout(resolve, attempt * 1000));
    }
  }
}
...
router.get('/api/tickmill-rate', async (req, res) => {
  try {
    ...
    const requestKey = `tickmill-${symbol}`;
    
    if (activeRequests.has(requestKey)) {
      console.log(`[Tickmill] Duplicate request blocked for ${symbol}`);
    ...
    try {
      console.log(`[Tickmill] Fetching rate for ${symbol} (ID: ${requestId})`);
      
      // Ensure query parameters are properly encoded
      const flaskUrl = `https://digit-tricks-dense-fundamental.trycloudflare.com/symbol_info?symbol=${encodeURIComponent(symbol)}&broker=tickmill`;
      console.log(`[Tickmill] URL: ${flaskUrl}`);
      ...
      console.log(`[Tickmill] Success for ${symbol}: bid=${data.bid}, ask=${data.ask}`);
      res.json(data);
      
    } catch (fetchError) {
      console.error(`[Tickmill] Failed for ${symbol}:`, fetchError.message);
      res.json({
        bid: 0,
        ask: 0,
        swap_long: 0,
    ...
    console.error('[Tickmill] Route error:', error);
