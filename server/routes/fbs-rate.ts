...
router.get('/api/fbs-rate', async (req, res) => {
  try {
    ...
    try {
      ...
      console.log(`[FBS] Fetching from Flask queue system: ${flaskUrl}`);
      ...
      if (!response.ok) {
        ...
        console.error(`[FBS] Flask HTTP error ${response.status}:`, errorText);
        throw new Error(`HTTP error! status: ${response.status} - ${errorText}`);
      }

      const data = await response.json();
      console.log('[FBS] Flask queue response:', data);
      
      // Check if Flask returned an error from the queue system
      if (data.error) {
        console.error('[FBS] Flask queue error:', data.error);
        res.json({
          bid: 0,
          ask: 0,
          swap_long: 0,
      ...
      console.error('[FBS] Fetch error:', fetchError.message);
      res.json({
        bid: 0,
        ask: 0,
        swap_long: 0,
        ...
        broker: "fbs",
        symbol: symbol,
        error: `Failed to fetch rate from FBS API: ${fetchError.message}`
      });
    }
  } catch (error) {
